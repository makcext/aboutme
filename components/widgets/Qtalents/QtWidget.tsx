          ...
          <Box component="article" sx={{ width: '100%', height: '100%', position: 'relative', p: 0 }}>
            <Card elevation={4} sx={{ minHeight: 500 }}>
              <Box p={1} justifyContent={'space-between'}>
                <Typography variant="body2">Designed and developed new features to meet user requirements and needs.  </Typography>
              </Box>
              ...
              <Box sx={{ display: 'flex', overflowY: 'auto', scrollSnapType: 'x mandatory', scrollbarWidth: '-moz-initial', '&::-webkit-scrollbar': { display: 'none' } }} onScroll={handleScroll(index)}>
                {project.images.map((image, imageIndex) => (
                  <CardMedia key={imageIndex} component="img" height="400px" image={`aboutme/qt/${image}.webp`} alt={project.title} sx={{ flex: 'none', scrollSnapAlign: 'start', objectFit: 'contain' }} />
                ))}
              </Box>

              <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', mt: 0, paddingTop: 0 }}>
                {project.images.map((_, dotIndex) => (
                  <Box key={dotIndex} sx={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: dotIndex === currentImage[index] ? 'info.main' : 'grey.500', mx: 0.5 }} />
                ))}
              </Box>
